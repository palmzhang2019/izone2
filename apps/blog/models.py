...
class CategoryHant(models.Model):
    ...
    def get_article_list(self):
        return ArticleHant.objects.filter(category=self)


class TagHant(models.Model):
    name = models.CharField('文章标签', max_length=20)
    slug = models.SlugField(unique=True)
    description = models.TextField('描述', max_length=240, default=settings.SITE_DESCRIPTION,
                                   help_text='用来作为SEO中description,长度参考SEO标准')
    ...
    def get_article_list(self):
        '''返回当前标签下所有发表的文章列表'''
        return ArticleHant.objects.filter(tags=self)


class KeywordHant(models.Model):
    name = models.CharField('文章关键词', max_length=20)
...
class ArticleHant(models.Model):
    ...
    def get_pre(self):
        return ArticleHant.objects.filter(id__lt=self.id).order_by('-id').first()
